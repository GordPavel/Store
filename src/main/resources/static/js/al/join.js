var Join = {
	...
	checkStatus: function (phone, cntr) {
		var box = curBox(), shprg = box ? box.showProgress : lockButton.pbind('join_send_phone'),
	...
	submitPhone: function (force) {
		...
		var phoneEl = ge('join_phone'), phone = Join.getPhone(), phoneInputVal = val('join_phone'),
	...
	updateStrength: function () {
		...
		if (pwd.length < 6) {
			...
			var groups = [/[^a-z]/g, /[^A-Z]/g, /[^�-��]/g, /[^�-ߨ]/g, /[^0-9]/g, /[a-zA-Z�-��-߸�0-9]/g], cnts = [],
				big = 0;
...
'
	...
	call: function () {
		ajax.post('join.php', {act: 'call', hash: cur.hash}, {
			progress: curBox().progress, onDone: function (text) {
				curBox().hide();
				Join.showMsg('join_submit_result', text);
				elfocus('join_code');
			}
	...
	submitEducation: function () {
		...
		if (cur.isSchool) {
			...
			params = ProfileEditorEdu.addSchoolParams(params, {id: 0}, 'school0');
			...
			params = ProfileEditorEdu.addUniParams(params, {id: 0}, 'primary_uni');
	...
	showPolicy: function (act) {
		showBox('al_help.php', {act: act, cc_id: cur.uiPhoneCountry.val() || 0, box: 1});
	},
	...
	updatePolicyLink: function (countryId) {
		...
		ajax.post('join.php', {act: 'update_policy', cc_id: countryId || 0}, {onDone: onDone});
	}
};
